...
public class C2W_SortColors extends Application {
    ...
    private  void bubbleSort(int n) {
    	for(int i=0;i<n-1;i++) {
	        ...
	        c2w_setHue(i,c2w_hue[i]); // Move c2w_hue[hi] into empty space.
            c2w_setHue(i,-1);      // Mark location hi as empty.
            c2w_delay(100);
    		
    		for(int j=0;j<n-i-1;j++) {
    			
    	        int tmp = c2w_hue[j];
    	        c2w_hue[j] = c2w_hue[j+1];
    	        c2w_hue[j] = tmp;
    	        
    	        c2w_setHue(j,c2w_hue[j+1]); // Move c2w_hue[hi] into empty space.
                c2w_setHue(j,-1);      // Mark location hi as empty.
                c2w_delay(100);
